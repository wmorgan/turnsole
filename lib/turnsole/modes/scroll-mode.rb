...
## a mode that can be scrolled horizonally and vertically
class ScrollMode < Mode
  ...
  attr_reader :topline, :botline, :leftcol
  ...
  def draw!
    ...
    ((@botline - @topline + 1) ... buffer.content_height).each do |ln|
      if @twiddles
        buffer.write ln, 0, "~", :color => :twiddle
      else
        buffer.write ln, 0, ""
      end
    end
  ...
  def status_bar_text
    "lines #{@topline + 1}:#{@botline}/#{num_lines}"
  end

  def continue_search_in_buffer
    unless @search_query
      @context.screen.minibuf.flash "No current search!"
      return
    end
    ...
    if line.nil? && (start > 0)
      ...
      @context.screen.minibuf.flash "Search wrapped to top!" if line
    end
    if line
      @search_line = line + 1
      search_goto_pos line, col, col + @search_query.display_width
      buffer.mark_dirty!
    else
      @context.screen.minibuf.flash "Not found!"
    end
  end
...
protected
  ...
  def find_text query, start_line
    regex = /#{query}/i
    (start_line ... num_lines).each do |i|
      case(s = self[i])
      when String
        match = s =~ regex
